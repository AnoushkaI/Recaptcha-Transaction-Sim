...
───────────────────────
Phase 3 verification tests (Async):
...
async def main():
    ...
    try:
        ...
        explanation = await agent.explain(SAMPLE_ALERT)
        ...
        report("InvestigatorAgent returns real text (not a stub)", is_real, f"Length: {len(explanation)} chars")
    ...
    # Test 2: RuleWriterAgent — valid request
    print(SEPARATOR)
    ...
    try:
        ...
        report("Code contains evaluate() function", "def evaluate(" in result["code"])
    ...
    # Test 3: RuleWriterAgent retry loop
    print(SEPARATOR)
    print("TEST 3: RuleWriterAgent — retry loop demo")
    print(SEPARATOR)
